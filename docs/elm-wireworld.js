...
var author$project$Matrix$repeat = F3(
	function (rowLen, colLen, value) {
		return {
			columnLength: colLen,
			data: A2(elm$core$Array$repeat, rowLen * colLen, value),
			rowLength: rowLen
		};
	});
...
var author$project$Main$Conductor = {$: 'Conductor'};
...
var author$project$Main$keyToMsg = function (key) {
	switch (key) {
		case 'ArrowUp':
			return author$project$Main$MoveCursor(author$project$Main$ArrowUp);
		case 'ArrowDown':
			return author$project$Main$MoveCursor(author$project$Main$ArrowDown);
		case 'ArrowLeft':
			return author$project$Main$MoveCursor(author$project$Main$ArrowLeft);
		case 'ArrowRight':
...
var author$project$Main$moveCursor = F2(
	function (direction, model) {
		...
		var colmax = author$project$Main$matrixColLength - 1;
		var _n0 = model.cursorPosn;
		var row = _n0.a;
		var col = _n0.b;
		switch (direction.$) {
			case 'ArrowUp':
				return (!row) ? _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(rowmax, col)
					}) : _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row - 1, col)
					});
			case 'ArrowDown':
				return _Utils_eq(row, rowmax) ? _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(0, col)
					}) : _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row + 1, col)
			...
			case 'ArrowLeft':
				return (!col) ? _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row, colmax)
					}) : _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row, col - 1)
					});
			default:
				return _Utils_eq(col, colmax) ? _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row, 0)
					}) : _Utils_update(
					model,
					{
						cursorPosn: _Utils_Tuple2(row, col + 1)
...
var author$project$Matrix$set = F4(
	function (row, col, value, mat) {
		return ((row < 0) || ((_Utils_cmp(row, mat.rowLength) > -1) || ((col < 0) || (_Utils_cmp(col, mat.columnLength) > -1)))) ? mat : _Utils_update(
			mat,
			{
				data: A3(elm$core$Array$set, (row * mat.columnLength) + col, value, mat.data)
			});
	});
var author$project$Main$setStateToCursorPosn = F2(
...
var author$project$Matrix$get = F3(
	function (row, col, mat) {
		return ((row < 0) || ((_Utils_cmp(row, mat.rowLength) > -1) || ((col < 0) || (_Utils_cmp(col, mat.columnLength) > -1)))) ? elm$core$Maybe$Nothing : A2(elm$core$Array$get, (row * mat.columnLength) + col, mat.data);
	});
...
var author$project$Main$neighborhoodHeadCount = F3(
				...
				function (diffrow) {
					return elm$core$List$sum(
						A2(
							elm$core$List$map,
							function (diffcol) {
								return ((!diffrow) && (!diffcol)) ? 0 : (_Utils_eq(
									A3(author$project$Matrix$get, row + diffrow, col + diffcol, matrix),
									elm$core$Maybe$Just(author$project$Main$Head)) ? 1 : 0);
							},
							_List_fromArray(
								[-1, 0, 1])));
				},
	...
	var updateCell = F3(
		function (row, col, centerstate) {
			switch (centerstate.$) {
				...
				case 'Tail':
					return author$project$Main$Conductor;
				default:
					var headCount = A3(author$project$Main$neighborhoodHeadCount, model.matrix, row, col);
					return ((headCount === 1) || (headCount === 2)) ? author$project$Main$Head : author$project$Main$Conductor;
			}
		});
	return _Utils_update(
		model,
...
var author$project$Main$update = F2(
	function (msg, model) {
		var when = F2(
			function (t, newmodel) {
				return t ? _Utils_Tuple2(newmodel, elm$core$Platform$Cmd$none) : _Utils_Tuple2(model, elm$core$Platform$Cmd$none);
			});
		switch (msg.$) {
			case 'Start':
				return _Utils_Tuple2(
			...
			case 'SetState':
				...
				return A2(
					when,
					_Utils_eq(model.appState, author$project$Main$Editing),
					A2(author$project$Main$setStateToCursorPosn, state, model));
			case 'MoveCursor':
				var direction = msg.a;
				return A2(
					when,
	...
	function (row, col) {
		return elm$core$String$concat(
			_List_fromArray(
				[
					'translate(',
					elm$core$String$fromFloat(col * author$project$Main$cellSize),
					',',
					elm$core$String$fromFloat(row * author$project$Main$cellSize),
					')'
				]));
	});
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					elm$core$String$fromFloat(author$project$Main$cellSize)),
	...
	return A2(
		...
		_Utils_ap(
			author$project$Matrix$toList(
				A2(author$project$Matrix$indexedMap, author$project$Main$viewCell, model.matrix)),
			_Utils_eq(model.appState, author$project$Main$Editing) ? _List_fromArray(
				[
